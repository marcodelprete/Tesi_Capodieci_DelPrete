...
static void AnalogSetDefault(void)
{
	...
	CY_SET_XTND_REG8((void CYFAR *)CYREG_OPAMP1_MX, 0x02u);
...
static uint8 CYXDATA * const CYCODE AMux_Cur__outerAddr[2] = {
	(uint8 CYXDATA *)CYREG_PRT0_AG, 
	(uint8 CYXDATA *)CYREG_DAC2_SW4, 
};

static const uint8 CYCODE AMux_Cur__outerMask[2] = {
	0x01u, 
	0x10u, 
};

static uint8 CYXDATA * const CYCODE AMux_Cur__innerAddr[1] = {
	(uint8 CYXDATA *)CYREG_SC2_SW0, 
};

static const uint8 CYCODE AMux_Cur__innerMask[1] = {
	0x10u, 
};

static const uint8 CYCODE AMux_Cur__innerMap[2] = {
	0u, 
	0u, 
};

static uint8 AMux_Cur__armsEnabled = 0u;
static uint8 AMux_Cur__refCnt[] = {0u};

/*******************************************************************************
* Function Name: AMux_Cur_Set
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void AMux_Cur_Set(uint8 channel)
{
	uint8 i;
	uint8 channelMask = (1UL << channel);
	if ((channel < 2u) && ((AMux_Cur__armsEnabled & channelMask) == ((uint32)0u)))
	{
		AMux_Cur__armsEnabled |= channelMask;
		*AMux_Cur__outerAddr[channel] |= AMux_Cur__outerMask[channel];
		i = AMux_Cur__innerMap[channel];
		if (AMux_Cur__refCnt[i] == 0u)
		{
			*AMux_Cur__innerAddr[i] |= AMux_Cur__innerMask[i];
		}
		AMux_Cur__refCnt[i]++;
	}
}
...
*
*******************************************************************************/
void AMux_Cur_Unset(uint8 channel)
{
	uint8 i;
	uint8 channelMask = (1UL << channel);
	if ((channel < 2u) && ((AMux_Cur__armsEnabled & channelMask) != ((uint32)0u)))
	{
		AMux_Cur__armsEnabled &= (uint8)~channelMask;
		*AMux_Cur__outerAddr[channel] &= (uint8)~AMux_Cur__outerMask[channel];
		i = AMux_Cur__innerMap[channel];
		AMux_Cur__refCnt[i]--;
		if (AMux_Cur__refCnt[i] == 0)
		{
			*AMux_Cur__innerAddr[i] &= (uint8)~AMux_Cur__innerMask[i];
		}
	}
}
...
uint8 CYXDATA * const CYCODE AMux_Electrodes__addrTable[2] = {
	(uint8 CYXDATA *)CYREG_OPAMP1_SW, 
	(uint8 CYXDATA *)CYREG_OPAMP1_SW, 
};

/* This is an implementation detail of the AMux. Code that depends on it may be
   incompatible with other versions of PSoC Creator. */
const uint8 CYCODE AMux_Electrodes__maskTable[2] = {
	0x01u, 
	0x02u, 
};

/*******************************************************************************
* Function Name: AMux_Electrodes_Set
********************************************************************************
* Summary:
*  This function is used to set a particular channel as active on the AMux.
*
...
*
*******************************************************************************/
void AMux_Electrodes_Set(uint8 channel)
{
	if (channel < 2)
	{
		*AMux_Electrodes__addrTable[channel] |= AMux_Electrodes__maskTable[channel];
	}
}

/*******************************************************************************
* Function Name: AMux_Electrodes_Unset
********************************************************************************
* Summary:
*  This function is used to clear a particular channel from being active on the
*  AMux.
...
*
*******************************************************************************/
void AMux_Electrodes_Unset(uint8 channel)
{
	if (channel < 2)
	{
		*AMux_Electrodes__addrTable[channel] &= (uint8)~AMux_Electrodes__maskTable[channel];
	}
}
